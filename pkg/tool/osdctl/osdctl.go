...
func NewTool() *Tool {
	t := &Tool{
		source: github.NewGithubSource("openshift", "osdctl"),
	}
	return t
}
...
func unArchive(source string, destination string) error {
	...
	if err != nil {
		return fmt.Errorf("failed to read the new gzip reader '%s': %v", source, err)
	}
	defer func() {
		err = uncompressed.Close()
		if err != nil {
			fmt.Printf("creating gzip.Reader '%s': %v", source, err)
		}
	}()
	...
	for {
		...
		if err != nil {
			if err != io.EOF {
				return fmt.Errorf("erroring about EOF: %v", err)
			}
			break
		}
		if f.FileInfo().IsDir() {
			...
			if err != nil {
				return fmt.Errorf("erroring about : %v", err)
			}
		} else {
			extractFile(destination, f, arc)
		}
	}
	return nil
}
...
func extractFile(destination string, f *tar.Header, arc io.Reader) error {
	...
	if err != nil {
		return fmt.Errorf("erroring when creating file: %v", err)
	}
	defer func() {
		err = dst.Close()
		if err != nil {
	...
	if err != nil {
		return fmt.Errorf("erroring when overriding file permission : %v", err)
	}
	_, err = dst.ReadFrom(arc)
	if err != nil {
		return fmt.Errorf("failed to read from arc file  %v", err)
	}
	return nil
}
